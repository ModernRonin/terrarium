        ...
        public static SimulationState Default
        {
            get
            {
                ...
                return new SimulationState(
                    new List<EntityState> {EntityState.Cross.At(new Vector2D(10, 10)), EntityState.Snake.At(new Vector2D(90, 90))},
                    new List<EnergySource> {new EnergySource(new Vector2D(50f, 50f), 25f, new Vector2D(-0.01f, -0.003f))},
                    size);
            }
        }
        public Vector2D Size { get; }
        public IEnumerable<EntityState> Entities { get; }
        public float[,] EnergyDensity { get; }
        public IEnumerable<EnergySource> EnergySources { get; }
        public IEnumerable<EntityState> GetEntitiesAt(Vector2D position)
        {
